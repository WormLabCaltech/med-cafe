...
% \affil[1]{Corresponding author. Contact: pws@caltech.edu}
% \renewcommand\Affilfont{\itshape\small{}}

\keywords{Genetic interactions; Transcriptome; Mediator; Wnt; Ras; Dominance;
          Cdk8}
\runningtitle{allelic series using transcriptomes} % For use in the footer
\correspondingauthor{Paul W. Sternberg}

\begin{abstract}
  Although transcriptomes have recently been used to perform epistasis analyses,
  they are not yet used to study intragenic function/structure relationships. We
  developed a theoretical framework to study allelic series using transcriptomic
  phenotypes. As a proof-of-concept, we apply our methods to an allelic series
  of \dpy{}, a highly pleiotropic \emph{Caenorhabditis~elegans} gene orthologous
  to the human gene \gene{MED12}, which encodes a subunit of the Mediator
  complex. Our methods identify functional units within \dpy{} that modulate
  Mediator activity upon various genetic programs, including the Wnt and Ras
  modules.
\end{abstract}
...
\correspondingauthoraffiliation{
        Division of Biology and Biological Engineering, Mail Code
        156--29, Biology Division, Caltech, 1200 E California Blvd.,
        Pasadena, California 91125. E-mail: pws@caltech.edu
        }
\vspace{-11pt}%
...
the Ras pathway). Homozygotes of allele \gene{dpy-22(bx93)}, which encodes a
premature stop codon Q2549Amber~\citep{Zhang2000}, appear grossly wild-type,
though this allele does not have complete wild-type functionality, since it
fails to fully complement the Muv phenotype of another allele, \emph{sy622}, in
a sensitized \emph{let-23} background. In contrast, animals homozygous for a
more severe allele, \gene{dpy-22(sy622)} encoding another premature stop codon,
...
amount of overlap that is biologically significant, rather than statistically
significant. In part, we reason that there is a non-trivial possibility that all
mutations are associated with a stress response, which we refer to as
genotoxicity. If that were the case, many mutants may share the same
transcriptomic signatures. To identify pathway enrichment, we would have to
identify this genotoxic signature and computationally remove it.
