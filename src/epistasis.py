...
def label(strain1, strain2):
    """A function to make epistasis labels given two strain-letters."""
    return '{0} $>$ {1}'.format(genvar.fancy_mapping[strain1],
...
def find_overlap(genotypes, df, q=0.1, col='strain'):
    ...
    for target, group in grouped:
        ...
        all_in = (len(group.strain.unique()) == len(genotypes))
        if all_in:
            genes += [target]
    return genes
...
def find_STP(single_muts, double_mut, df, q=0.1):
    ...
    double_muts - a strain for a double mutant
    df - a tidy dataframe. must contain columns 'target_id' and 'strain'
...
def epiplot(X, Y, Y_se, **kwargs):
    ...
    if len(X) != len(Y):
        raise ValueError('Lengths of X, Y and Y_se must match')
...
def make_epiplot(singles, double, df, **kwargs):
    ...
    if len(x) != len(y):
        raise ValueError('find_STP threw a bug')
    if len(x) != len(xy):
        raise ValueError('find_STP threw a bug')
    if len(y) != len(xy):
        raise ValueError('find_STP threw a bug')
